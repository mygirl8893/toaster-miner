...
static void cryptoupx_store_variant(void* state, int variant) {
	if (variant == 1) {
...
struct cryptonight_ctx {
	...
	uint8_t _ALIGN(16) d[AES_BLOCK_SIZE];
	uint8_t _ALIGN(16) e[AES_BLOCK_SIZE];
	oaes_ctx* aes_ctx;
};

static void cryptoupx_hash_ctx(void* output, const void* input, int len, struct cryptonight_ctx* ctx, int variant)
{
	size_t i, j;
	...
	for (i = 0; likely(i < ITER / 4); ++i) {
		...
		/* Iteration 2 */
		cryptoupx_store_variant(&ctx->long_state[j], variant);
...
void cryptoupx_hash(void* output, const void* input) {
	const int variant = 1;
	struct cryptonight_ctx *ctx = (struct cryptonight_ctx*)malloc(sizeof(struct cryptonight_ctx));
	cryptoupx_hash_ctx(output, input, 76, ctx, variant);
	free(ctx);
}

static void cryptoupx_hash_ctx_aes_ni(void* output, const void* input, int len, struct cryptonight_ctx* ctx, int variant)
{
	size_t i, j;
...
int scanhash_cryptoupx(int thr_id, struct work *work, uint32_t max_nonce, uint64_t *hashes_done)
{
	...
	if (aes_ni_supported) {
		do {
			*nonceptr = ++n;
			cryptoupx_hash_ctx_aes_ni(hash, pdata, 76, ctx, variant);
		...
		do {
			*nonceptr = ++n;
			cryptoupx_hash_ctx(hash, pdata, 76, ctx, variant);
			if (unlikely(hash[7] < ptarget[7])) {
				work_set_target_ratio(work, hash);
				*hashes_done = n - first_nonce + 1;
				free(ctx);
