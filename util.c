...
void get_defconfig_path(char *out, size_t bufsize, char *argv0)
{
	...
	struct stat info = { 0 };
#ifdef WIN32
	snprintf(out, bufsize, "%s\\toasterminer\\toasterminer-conf.json", getenv("APPDATA"));
#else
	snprintf(out, bufsize, "%s\\.toasterminer\\toasterminer-conf.json", getenv("HOME"));
#endif
	if (dir && stat(out, &info) != 0) {
		snprintf(out, bufsize, "%s%stoasterminer-conf.json", dir, sep);
	}
	if (stat(out, &info) != 0) {
		out[0] = '\0';
		return;
...
void print_hash_tests(void)
{
	...
	cryptoupx_hash(&hash[0], &buf[0]);
	printpfx("cryptoupx", hash);
	/*
