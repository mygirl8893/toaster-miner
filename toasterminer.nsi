...
!define PROGRAM_KEY  "toasterminer"

Name "toasterminer-multi v${MINER_VERSION}"
...
!ifdef NSIS_MAKENSIS64
  !define BITS 64
  InstallDir $PROGRAMFILES64\toasterminer-multi
  !define RK_UNINSTALL "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\${PROGRAM_KEY}"
!else
  !define BITS 32
  InstallDir $PROGRAMFILES32\toasterminer-multi
  !define RK_UNINSTALL "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\${PROGRAM_KEY}"
...
Function func_title_pre1    ; Page 1, Pre
  ...
  SendMessage $_2_ ${WM_SETTEXT} 0 STR:$(LSTR_39)    ;  Please wait while ${PROGRAM_NAME} is being installed. toasterminer-multi
FunctionEnd
...
Section
  ...
  SetOverwrite on
  File toasterminer-gw64.exe
  File toasterminer-x64.exe
  File toasterminer-conf.json
  ...
  File "/oname=$DATADIR\toasterminer-conf.json" toasterminer-conf.json
  WriteUninstaller "$DATADIR\toasterminer-uninst.exe"

  # Shortcuts (program + uninstaller)
  CreateDirectory "$SMPROGRAMS\${PROGRAM_NAME}"
  CreateShortCut /NoWorkingDir "$SMPROGRAMS\${PROGRAM_NAME}\${PROGRAM_NAME}.lnk" "$REALINSTDIR\toasterminer-gw64.exe"
  CreateShortCut /NoWorkingDir "$SMPROGRAMS\${PROGRAM_NAME}\Config.lnk" "$SYSDIR\notepad.exe" "$DATADIR\toasterminer-conf.json"
  CreateShortCut "$SMPROGRAMS\${PROGRAM_NAME}\Uninstall.lnk" "$DATADIR\toasterminer-uninst.exe"
  CreateShortCut /NoWorkingDir "$SMPROGRAMS\${PROGRAM_NAME}\${PROGRAM_NAME}-bg.lnk" "$REALINSTDIR\toasterminer-gw64.exe" "-q -B" "" "" SW_SHOWMINIMIZED
  ...
  WriteRegStr HKLM ${RK_UNINSTALL} \
    "DisplayIcon" "$REALINSTDIR\toasterminer-x64.exe"

  WriteRegStr HKLM ${RK_UNINSTALL} \
    "InstallLocation" "$REALINSTDIR"

  WriteRegStr HKLM ${RK_UNINSTALL} \
    "UninstallString" "$\"$DATADIR\toasterminer-uninst.exe$\""

  ${GetSize} "$INSTDIR" "/M=toasterminer* /S=0K /G=0" $0 $1 $2
  IntFmt $0 "0x%08X" $0
  WriteRegDWORD HKLM "${RK_UNINSTALL}" \
    "EstimatedSize" "$0"

  # Add application to Windows Firewall exception list (to check)
  ;liteFirewall::AddRule "$REALINSTDIR\toasterminer-gw64.exe" "CPU Miner (MinGW64)"
  ;liteFirewall::AddRule "$REALINSTDIR\toasterminer-x64.exe" "CPU Miner (x64)"
...
Section "uninstall"
  ...
  Delete "$INSTDIR\toasterminer-conf.json"
  Delete "$INSTDIR\toasterminer-gw64.exe"
  Delete "$INSTDIR\toasterminer-x64.exe"
  ...
  Delete "$DATADIR\toasterminer-uninst.exe"
  RMDir "$DATADIR"

  ; Delete "$DATADIR\toasterminer-conf.json"
  ; RMDir "$DATADIR"
  ...
  ;liteFirewall::RemoveRule "$REALINSTDIR\toasterminer-gw64.exe" "CPU Miner (MinGW64)"
  ;liteFirewall::RemoveRule "$REALINSTDIR\toasterminer-x64.exe" "CPU Miner (x64)"
